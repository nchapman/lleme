...
func TestBackendsCredit(t *testing.T) {
	tests := []struct {
		...
		{name: "tag present", llama: "b1234", wantSubstr: []string{"llama.cpp b1234"}},
		{name: "empty tag", wantEmpty: true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := BackendsCredit(tt.llama)
			if tt.wantEmpty {
				if got != "" {
			...
			// Never print a naked "llama.cpp " with no tag.
