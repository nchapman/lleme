...
func TestMergeServerOptionsLayering(t *testing.T) {
	p := &Preset{
		Options: map[string]any{"temp": 0.6, "mirostat": 2},
	}
	...
	if got["mirostat"] != 2 {
		t.Errorf("mirostat = %v, want 2 (from preset)", got["mirostat"])
	}
...
// Persona options (already folded into personaOpts by the caller via
// Persona.GetServerOptions) must win over the preset's own layer. This is
// the load-bearing guarantee of the layered precedence.
func TestMergeServerOptionsPersonaOverridesPreset(t *testing.T) {
	p := &Preset{
		Options: map[string]any{"mirostat": 2},
	}
	personaOpts := map[string]any{"mirostat": 1}

	got := MergeServerOptions(p, personaOpts)
	if got["mirostat"] != 1 {
		t.Errorf("mirostat = %v, want 1 (persona beats preset)", got["mirostat"])
	}
}
