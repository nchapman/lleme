...
Keys under `options` mirror `llama-server` CLI flags in kebab-case (`--temp` → `temp`, `--top-k` → `top-k`, `--presence-penalty` → `presence-penalty`, etc.). llama-server passes unknown keys through verbatim.
