...
Models are loaded on-demand and unloaded after idle timeout.`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		// only download the runtime we actually need. See Step 2 below.
		...
		// Fail fast on legacy MLX models instead of downloading llama.cpp
		// and erroring later at load time.
		if resolvedModel.Backend == hf.BackendMLX {
			ui.Fatal("MLX models are no longer supported (the SwiftLM backend was removed); pull the GGUF build of this model instead")
		}

		// Install llama.cpp if missing. validateModel may have just pulled
		// the model, so check after resolution.
